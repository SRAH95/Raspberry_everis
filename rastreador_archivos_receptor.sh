...
rastreador()
{
    ...
    for j in $(ls $path_rasppi); do
        ...
        if [ "b"=="1" ]; then
            wget -m ftp://everis:IFEGSA@192.168.3.1/DSK1/SSN/LOG1_everis_prueba_RINEX_24H/$j    #El "path" que se da en las lineas 29 y 31 es la del receptor.
        elif [ "d"=="0"]; then
            wget -m ftp://everis:IFEGSA@192.168.3.1/DSK1/SSN/LOG1_everis_prueba_RINEX_24H/$j
        fi
    done
